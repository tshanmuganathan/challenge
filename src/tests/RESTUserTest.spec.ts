...
test(`TC001 - Get Weather Details`, async ({ gData }) => {
    ...
    await user.verifyContentIsNotNull(response, '$.location.name', 'Location Name');
    let tempInCelcius = await user.extractResponseValue(response, '$.current.temp_c', 'Temperature in Celcius');
    gData.set("Temperature in Celcius", tempInCelcius);
});
